...
export default async function SearchPage({
    ...
    if (params.rating && params.rating !== "all") {
        activeFilters.push({ label: "Rating", value: `${params.rating} stars & up` });
    }
    if (params.sort) {
        ...
        <div className="min-h-screen p-6 md:p-10 flex flex-col md:flex-row gap-4">
            <section className="w-full md:max-w-xs flex flex-col gap-6 ">
                <div>
                    ...
                    <div className="flex flex-col gap-2">
                        {categories.map((cat) => (
                            <Link
                                key={cat.category}
                                href={{ pathname: "/search", query: buildQuery(params, { category: cat.category }) }}
                        ...
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { category: "all" }) }}
                            className={!params.category || params.category === "all" ? "font-bold" : ""}
                        >
                            All categories
                        </Link>
                    </div>
                ...
                <div>
                    ...
                    <div className="flex flex-col gap-2">
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { price: "1-50" }) }}
                            className={params.price === "1-50" ? "font-bold" : ""}
                        >
                            1 € to 50 €
                        </Link>
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { price: "51-100" }) }}
                            className={params.price === "51-100" ? "font-bold" : ""}
                        >
                            51 € to 100 €
                        </Link>
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { price: "101-200" }) }}
                            className={params.price === "101-200" ? "font-bold" : ""}
                        >
                            101 € to 200 €
                        </Link>
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { price: "501-1000" }) }}
                            className={params.price === "501-1000" ? "font-bold" : ""}
                        >
                            501 € to 1000 €
                        </Link>
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { price: "all" }) }}
                            className={!params.price || params.price === "all" ? "font-bold" : ""}
                        >
                            All prices
                        </Link>
                    </div>
                ...
                <div>
                    ...
                    <div className="flex flex-col gap-2">
                        {["4", "3", "2", "1"].map((value) => (
                            <Link
                                key={value}
                                href={{ pathname: "/search", query: buildQuery(params, { rating: value }) }}
                        ...
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { rating: "all" }) }}
                            className={!params.rating || params.rating === "all" ? "font-bold" : ""}
                        >
                            All ratings
                        </Link>
                    </div>
            ...
            <section className="flex-1 ">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-4">
                    ...
                    <div className="flex flex-wrap gap-4 text-sm font-medium">
                        ...
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { sort: "newest" }) }}
                            className={params.sort === "newest" ? "font-bold underline" : ""}
                        >
                            NEWEST
                        </Link>
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { sort: "price-low" }) }}
                            className={params.sort === "price-low" ? "font-bold underline" : ""}
                        >
                            LOWER
                        </Link>
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { sort: "price-high" }) }}
                            className={params.sort === "price-high" ? "font-bold underline" : ""}
                        >
                            HIGHER
                        </Link>
                        <Link
                            href={{ pathname: "/search", query: buildQuery(params, { sort: "rating-high" }) }}
                            className={params.sort === "rating-high" ? "font-bold underline" : ""}
                        >
                            RATING
                        </Link>
                    </div>
